...
template <typename T>
static void merge_defaults(std::vector<T> &dst,
                           const std::vector<T> &defaults) {
  if (dst.empty())
    dst = defaults;
  else
    dst.insert(dst.end(), defaults.begin(), defaults.end());
}

inline std::vector<CapyString>
make_capy_vector(std::initializer_list<const char *> strings) {
...
CAPY_STRING_VECTOR(s_IGNORED_NAMESPACES, "std", "__gnu", "<anon>", "1", "2",
                   "3", "4", "5", "6", "7", "8", "9", "0", "_IO_", "_G_");
CAPY_STRING_VECTOR(s_IGNORED_CLASSNAMES, "std", "__gnu", "<anon>", "1", "2",
                   "3", "4", "5", "6", "7", "8", "9", "0", "_IO_", "_G_",
                   "__pthread", "timespec", "lconv", "_M_", "tm",
                   "typedef __va_");
CAPY_STRING_VECTOR(s_IGNORED_NAMES, "<anon>", "gp_offset", "fp_offset",
                   "overflow_arg_area", "reg_save_area", "tm_", "__", "_IO",
                   "_flags", "quot", "_markers", "_chain", "_short",
                   "_old_offset", "_cur_column", "_vtable_offset", "_shortbuf",
                   "_lock", "_offset", "_codecvt", "_wide_data", "_freeres",
                   "_prevchain", "_mode", "_unused", "_total_written",
                   "decimal_point", "thousands_sep", "goruping",
                   "int_curr_symbol", "currency_symbol", "mon_decimal_point",
                   "mon_thousands_sep", "mon_grouping", "positive_sign",
                   "negative_sign", "int_frac_digits", "frac_digits",
                   "p_cs_precedes", "p_sep_by_space", "n_cs_", "n_sep_",
                   "p_sign_", "n_sign_", "int_p_", "int_n_", "rem", "_fileno",
                   "grouping");

static inline CapyString
join_names(const std::vector<CapyString> &scope_stack) {
...
static std::string
jit_get_compile_command(const std::filesystem::path &filePath) {
  ...
  std::filesystem::path compilePath =
      rootDir / g_Storage.Config.BinaryPath / filePath.filename();
  ...
  if (jit.JitCompilationNeeded.exchange(false)) {
    ...
    for (auto &path : files_being_compiled) {
      std::filesystem::path objPath =
          rootDir / g_Storage.Config.BinaryPath / path.filename();
...

    {
      ...
      ss << "-o " << g_Storage.Config.BinaryPath.generic_string()
         << "/CapyBinary.so \\\n";
...
void capy_set_source_path(const std::filesystem::path &sourcePath,
                          FileEventCallback callback, bool recursive) {
  auto &jit = g_Storage.Active.JITStorage;
...
void capy_set_ignored_namespace(
    const std::vector<CapyString> &ignoredNamespace) {
  for (auto &nameSpace : ignoredNamespace)
    g_Storage.Config.IgnoredNamespaces.push_back(nameSpace);
}

void capy_set_ignored_classname(
    const std::vector<CapyString> &ignoredClassName) {
...
static std::string dump_class(CapyClass *klass, bool isStruct) {
  std::string str;
  std::string adjustedClassType =
      isStruct ? "  Full Struct Name: " : "  Full Class Name: ";
  std::string adjustedClassName =
      klass->ClassName.empty() ? "<Functions Only>" : klass->ClassName.c_str();
  str.append(adjustedClassType + adjustedClassName +
             " (Total Size: " + std::to_string(klass->ClassSize) + ")");
  ...
  std::unordered_map<uint64_t, CapyTableMetaData> methodTable =
      capy_table_info_get(klass->BaseImage, CapyTableType::MethodDef)
          ->Symbols[klass->Hash];
  std::unordered_map<uint64_t, CapyTableMetaData> fieldTable =
      capy_table_info_get(klass->BaseImage, CapyTableType::FieldDef)
          ->Symbols[klass->Hash];
  ...
  for (auto &[hash, sym] : methodTable) {
    ...
    str.append("    (METHOD) ")
        .append(sym.ReturnType.c_str())
        .append(" " + adjustedSymName + "\n");
  }
...
void capy_resolve_class_layout(CapyClass *klass) {
  ...
  for (auto &base : g_Storage.Config.ClassMap[klass->Hash]) {
    CapyClass *baseClass =
        capy_class_from_name(base.NameSpace.c_str(), base.ClassName.c_str());
    if (!baseClass)
      continue;
    ...
    for (auto &[fieldHash, baseField] : baseClass->VTable->Fields) {
      ...
      klass->SubFields[fieldHash] = {copiedField->Size,
                                     copiedField->Offset +
                                         (baseSize - baseClass->ClassSize)};
    }
  ...
  for (auto *field : fields) {
    if (field->Size == 0) {
      CapyClass *type = capy_class_from_name(klass->NameSpace.c_str(),
                                             field->FieldTypeString.c_str());
      if (type) {
...
void capy_reload_libraries_into_domain() {
  ...
  for (auto &entry :
       std::filesystem::directory_iterator(g_Storage.Config.BinaryPath)) {
...
CapyLibrary *capy_domain_library_open(const std::string &binName, bool isCore) {
  ...
  for (auto &cu : dw.compilation_units())
    traverse_and_collect(cu.root(), *(new std::vector<CapyString>), g_Storage,
                         symbols);
  ...
  for (auto &sym : symbols) {
    ...
    uint64_t classHash = make_symbol_hash(
        {capy_string_intern(sym.Namespace), capy_string_intern(sym.ClassName)});
    ...
    if (!found) {
      ...
      CapyTableInfo *table =
          capy_table_info_get(image.get(), CapyTableType::TypeDef);

      CapyTableMetaData tableMetaData = {
          sym.Namespace, sym.ClassName, CapyString(), CapyString(),
          false,         false,         sym.IsStruct};
    ...
    if (sym.IsVariable) {
      ...
      CapyTableInfo *table =
          capy_table_info_get(image.get(), CapyTableType::FieldDef);
      CapyTableMetaData metaData = {
          sym.Namespace,  sym.ClassName,       sym.Name,    sym.ReturnType,
          sym.IsVariable, sym.IsClassInstance, sym.IsStruct};
      ...
      CapyTableInfo *table =
          capy_table_info_get(image.get(), CapyTableType::MethodDef);
      CapyTableMetaData metaData = {
          sym.Namespace,  sym.ClassName,       sym.Name,    sym.ReturnType,
          sym.IsVariable, sym.IsClassInstance, sym.IsStruct};
  ...
  std::unique_ptr<CapyLibrary> library =
      std::make_unique<CapyLibrary>(std::move(image));
...
CapyClass *capy_class_from_name(CapyImage *i, const std::string &nameSpace,
...
CapyClass *capy_class_from_name(const std::string &nameSpace,
                                const std::string &className) {
  std::string fullName;
  if (!nameSpace.empty())
    fullName += nameSpace + "::";
...
CapyMethod *capy_method_from_class(CapyClass *c,
                                   const std::string &functionName) {
  uint64_t funcHash = make_symbol_hash(
      {c->NameSpace, c->ClassName, capy_string_intern(functionName.c_str())});
  return c->VTable->Methods[funcHash].get();
}

CapyField *capy_field_from_class(CapyClass *c, const std::string &fieldName) {
  uint64_t fieldHash = make_symbol_hash(
      {c->NameSpace, c->ClassName, capy_string_intern(fieldName.c_str())});
...
static bool resolve_field(CapyClass *cc, const std::string &fieldName,
                          FieldResolveResult &out) {
  ...
  if (dotOperator != std::string::npos) {
    ...
    subFieldName =
        capy_string_intern(fieldName.substr(dotOperator + 1).c_str());
  }

  CapyString adjustedFieldName = capy_string_intern(initailFieldName.c_str());
  uint64_t fieldHash =
      make_symbol_hash({cc->NameSpace, cc->ClassName, adjustedFieldName});
  ...
  if (cf && subFieldName.empty()) {
    ...
    uint64_t fieldHash =
        make_symbol_hash({nameSpace, returnType, subFieldName});
    ...
    for (auto &[_, baseClass] : cc->BaseClasses) {
      uint64_t fieldHash = make_symbol_hash(
          {baseClass.NameSpace, baseClass.ClassName, subFieldName});
...
void capy_field_data_get(CapyObject *instance, CapyClass *cc,
                         const std::string &fieldName, void *value) {
  auto *cd = g_Storage.Active.Runtime.get();
  ...
  if (!resolve_field(cc, fieldName, res))
    return;
...
void capy_field_data_set(CapyObject *instance, CapyClass *cc,
                         const std::string &fieldName, void *value) {
  auto *cd = g_Storage.Active.Runtime.get();
...
void *capy_function_call_from_method(CapyMethod *method,
  ...
  if (ffi_prep_cif(&cif, FFI_DEFAULT_ABI, nargs, ffiRet, ffiArgTypes.data()) !=
      FFI_OK) {
  ...
  if (method->ClassMember) {
    using MemberFn = void (*)(...);
    ffi_call(&cif, FFI_FN(reinterpret_cast<MemberFn>(method->SymHandle)), &ret,
             ffiArgValues.data());
  } else {
...
void capy_add_internal_call(const std::string &name, void *functionSymbol) {
  ...
  if (storage.InternalCalls.contains(callHash))
    return;
      ...
      for (auto &[symHash, sym] : cls->VTable->Fields) {
        ...
        if (symHash == nameHash) {
          ...
          void **addr = reinterpret_cast<void **>(
              cls->VTable->Fields[symHash]->SymHandle);
          if (addr)
            *addr = functionSymbol;
        }
      }
...
CapyObject *capy_instantiate_object(CapyClass *klass) {
  ...
  size_t derivedOwnSize =
      klass->ClassSize - klass->BaseClassSize; // fields added in derived
  size_t offset = (klass->BaseClassSize + CLASS_ALIGNMENT - 1) &
                  ~(CLASS_ALIGNMENT - 1); // base -> derived padding

  size_t totalSize =
      (offset + derivedOwnSize + CLASS_ALIGNMENT - 1) & ~(CLASS_ALIGNMENT - 1);
